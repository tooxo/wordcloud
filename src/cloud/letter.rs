...
impl Letter {
    pub fn new(char: char, pixel_bounding_box: Rect<f32>, offset: Point<f32>, rotation: Rotation) -> Self {
        Self { char, pixel_bounding_box, offset, cursor: Point::default(), state: Vec::default(), simplified_state: None, rotation }
    }

    pub(crate) fn relative_bounding_box(&self, rotation: &Rotation) -> Rect<f32> {
        let bbox = self.pixel_bounding_box;

        rotation.rotate_rectangle(bbox + Point { x: 0.0, y: self.offset.y })
    }

    pub(crate) fn absolute_collidables(&self, rotation: &Rotation, word_offset: Point<f32>) -> Vec<Line<f32>> {
